...
static int	validate_and_store_rgb(char **rgb_tab_sanitized, t_map_data *map_data, const char *type)
{
	int	i;
	int	value;
	int	is_valid;
...
static int	parse_rgb_values(char *rgb_str_sanitized, t_map_data *map_data, const char *type)
{
...
static void	validate_color_line(char *line, char *line_origin, int fd, t_map_data *map_data)
{
	if (line[0] != ' ')
	{
		free(line_origin);
