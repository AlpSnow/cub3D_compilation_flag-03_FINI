...
// la codam stocke les pixels des images dans un tableau tout bete, pas de deux dimensions.
// on fait &img->pixels pour obtenir le pixel.
// on veut la ligne y = 0 et x = 6 :   
...
// j'ai eu des problemes avec les couleurs donc on est passé en gros d'un char* a un tab de int durant le parsing
// Puis je l'ai convertis en RGBA pour avoir les bonnes couleurs pour le ciel et sol (uint32_t)
// Et maintenant on passe sur du ARGB
// Conversion en ARGB : [AAAA AAAA][RRRR RRRR][GGGG GGGG][BBBB BBBB]
 ...
 *
 * Exemple :
 * Entrée : 0xFF0000FF (Rouge vif avec alpha 255)
 * Sortie : 0x7F0000FF (Rouge assombri avec alpha inchangé)
 ...
 *
 * Paramètres :
 *  - imgage       : pointeur vers l'image MLX42 où écrire
 *  - screen_x     : position horizontale du pixel
 *  - screen_y     : position verticale du pixel
 *  - color_argb   : couleur du pixel au format ARGB (32 bits)
 *
 * Conditions :
 ...
 *
 * Exemple :
 *  safe_put_pixel(screen, 100, 50, 0xFFFF0000); // Pixel rouge opaque
 */
